from utils import in_notebook, utcToChi, getChiTimeNow, readCSV, tqdm
...
def createConfigDir(comb_row, sec1_dir, day_chg_incs, minute_incs):
    ...
    with open(summary_sheet_fn, 'a+') as summary_sheet_file:
        summary_sheet_file.write('<h1>Best Models Summary</h1>')
        summary_sheet_file.write('<h1>Model Progress Summary</h1>')
        summary_sheet_file.write('<h1>Progress Details</h1>')
...
class DataObj:
    def __init__(self, config_dir,
        ...
        if (train_close_pricesDF is not None and trainDF is not None and
            ...
            print('loading train_close_pricesDF')
            self.train_close_pricesDF = readCSV(config_dir+'Postprocessed_Data/train_close_prices.csv', nrows=nrows)
            print('loading trainDF')
            self.trainDF = readCSV(config_dir+'Postprocessed_Data/train_minutesDF.csv', nrows=nrows)
            print('load train complete')

            if load_val:
                print('loading val_close_pricesDF')
                self.val_close_pricesDF = readCSV(config_dir+'Postprocessed_Data/val_close_prices.csv', nrows=nrows)
                print('loading valDF')
                self.valDF = readCSV(config_dir+'Postprocessed_Data/val_minutesDF.csv', nrows=nrows)
